    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Formulir Sunting Jadwal</h6>
                        </div>
                        <div class="card-body">
                            <?php 
                                $query = mysqli_query($conn, "SELECT * FROM jadwal WHERE kode_jadwal = '$kode_jadwal'");
                                $row = mysqli_fetch_array($query);
                            ?>
                            <form class="user" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <select class="custom-select" name="nama_hari" required>
                                        <option value="" disabled selected>Pilih Hari</option>
                                        <option value="Monday"
                                            <?php if ($row[1] == 'Monday') echo ' selected="selected"'; ?>>Senin
                                        </option>
                                        <option value="Tuesday"
                                            <?php if ($row[1] == 'Tuesday') echo ' selected="selected"'; ?>>Selasa
                                        </option>
                                        <option value="Wednesday"
                                            <?php if ($row[1] == 'Wednesday') echo ' selected="selected"'; ?>>Rabu
                                        </option>
                                        <option value="Thursday"
                                            <?php if ($row[1] == 'Thursday') echo ' selected="selected"'; ?>>Kamis
                                        </option>
                                        <option value="Friday"
                                            <?php if ($row[1] == 'Friday') echo ' selected="selected"'; ?>>Jum'at
                                        </option>
                                        <option value="Saturday"
                                            <?php if ($row[1] == 'Saturday') echo ' selected="selected"'; ?>>Sabtu
                                        </option>
                                    </select>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="text" class="form-control" placeholder="Jam Masuk"
                                            onfocus="(this.type='time')" onblur="(this.type='time')" required
                                            name="jam_masuk" value="<?php echo $row[2]; ?>">
                                    </div>
                                    <div class="col-sm-6">
                                        <input class="form-control" placeholder="Jam Keluar"
                                            onfocus="(this.type='time')" onblur="(this.type='time')" required
                                            name="jam_keluar" value="<?php echo $row[3]; ?>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <select class="custom-select" name="nik_guru" required>
                                        <option value="" disabled selected>Pilih Guru untuk Mengajar</option>
                                        <?php
                    $hasil = mysqli_query($conn, "SELECT * FROM guru WHERE jenis_akun <> 9 ORDER BY nama_guru ASC");
                    while ($baris = mysqli_fetch_array($hasil)) {
                      ?>
                                        <option value="<?php echo $baris['nik']; ?>"
                                            <?php if ($row[4] == $baris['nik']) echo ' selected="selected"'; ?>>
                                            <?php echo $baris['nama_guru']; ?>
                                        </option>
                                        <?php
                    }
                  ?>
                                ...
                                <div class="form-group">
                                    <select class="custom-select" name="mata_pelajaran" required>
                                        <option value="" disabled selected>Pilih Mata Pelajaran</option>
                                        <?php
                    $hasil = mysqli_query($conn, "SELECT * FROM mata_pelajaran ORDER BY nama_mata_pelajaran ASC");
                    while ($baris = mysqli_fetch_array($hasil)) {
                      ?>
                                        <option value="<?php echo $baris['kode_mata_pelajaran']; ?>"
                                            <?php if ($row[5] == $baris['kode_mata_pelajaran']) echo ' selected="selected"'; ?>>
                                            <?php echo $baris['nama_mata_pelajaran']; ?></option>
                                        <?php
                    }
                  ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    <select class="custom-select" name="kode_kelas" required>
                                        <option value="" disabled selected>Pilih Kelas untuk Diajar</option>
                                        <?php
                    $hasil = mysqli_query($conn, "SELECT * FROM kelas ORDER BY kode_kelas ASC");
                    while ($baris = mysqli_fetch_array($hasil)) {
                      ?>
                                        <option value="<?php echo $baris['kode_kelas']; ?>"
                                            <?php if ($row[6] == $baris['kode_kelas']) echo ' selected="selected"'; ?>>
                  ...
                  ?>
                                ...
                                <div class="form-group">
                                    <input type="submit" class="btn btn-primary btn-user btn-block" name="update_jadwal"
                                        value="Ubah Jadwal">
                                </div>
                            </form>
                        </div>
            ...
            if (isset($_POST['update_jadwal'])) {
              $nama_hari = antiinjection($_POST['nama_hari']);
              $jam_masuk = antiinjection($_POST['jam_masuk']);
              $jam_keluar = antiinjection($_POST['jam_keluar']);
              $nik_guru = antiinjection($_POST['nik_guru']);
              $kode_kelas = antiinjection($_POST['kode_kelas']);
              $kode_mata_pelajaran = antiinjection($_POST['mata_pelajaran']);

              $update = mysqli_query($conn, "UPDATE jadwal SET nama_hari = '$nama_hari', jam_masuk = '$jam_masuk', jam_keluar = '$jam_keluar', nik = '$nik_guru', kode_mata_pelajaran = '$kode_mata_pelajaran', kode_kelas = '$kode_kelas' WHERE kode_jadwal = '$kode_jadwal'");
              if ($update) {
                echo "<script>alert('Berhasil Mengubah Jadwal')</script>";
              } else {
