            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        ...
                        <a href="download_file_siswa.php" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                            ...
                            <form class="user" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <div class="custom-file">
                                        ...
                                        <label class="custom-file-label">Import File Data Siswa/i</label>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <input type="submit" class="btn btn-primary btn-user btn-block" name="input_siswa"
                                        value="Import Data Siswa/i">
                                </div>
                            </form>
                        </div>
...
if (isset($_POST['input_siswa'])) {
        ...
        foreach ($sheet as $row) {
            $nisn = antiinjection($row['A']);
            $nama_siswa = antiinjection($row['B']);
            $nis = antiinjection($row['C']);
            $gender = antiinjection($row['D']);
            $tempat_lahir = antiinjection($row['E']);
            $tanggal_lahir = antiinjection($row['F']);
            $alamat_siswa = antiinjection($row['G']);
            $nama_ayah = antiinjection($row['H']);
            $nama_ibu = antiinjection($row['I']);
            $pekerjaan_ayah = antiinjection($row['J']);
            $pekerjaan_ibu = antiinjection($row['K']);
            $jalan = antiinjection($row['L']);
            $kelurahan = antiinjection($row['M']);
            $kecamatan = antiinjection($row['N']);
            $kabupaten = antiinjection($row['O']);
            $provinsi = antiinjection($row['P']);
            $nama_wali = antiinjection($row['Q']);
            $alamat_wali = antiinjection($row['R']);
            $pekerjaan_wali = antiinjection($row['S']);
            $kelas_siswa = antiinjection($row['T']);
            if($nisn == "" && $nis == "" && $gender == "" && $tempat_lahir == "" && $tanggal_lahir == "" && $alamat_siswa == "" && $nama_ayah == "" && $nama_ibu == ""&& $pekerjaan_ayah == "" && $pekerjaan_ibu == "" && $jalan == "" && $kelurahan == "" && $kecamatan == "" && $kabupaten == "" && $provinsi == "" && $kelas_siswa == "")
            continue;
            if ($sheetcount > 2) {
                $tanggal = str_replace('/', '-', $tanggal_lahir);
                $date = date("d-m-Y", strtotime($tanggal));
                mysqli_query($conn, "INSERT INTO siswa VALUES('$nisn', '$nama_siswa', '$nis', '$gender', '$tempat_lahir', '$date', '$alamat_siswa', '$nama_ayah', '$nama_ibu', '$pekerjaan_ayah', '$pekerjaan_ibu', '$jalan', '$kelurahan', '$kecamatan', '$kabupaten', '$provinsi', '$nama_wali', '$pekerjaan_wali', '$alamat_wali', '$hashed_password', 1, '$kelas_siswa')");
            }
            $sheetcount++;
        }
