...
if (isset($_SESSION['login'])) {
    ...
    $kode_walikelas = $_GET['kode_walikelas'];
  ?>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Formulir Sunting Wali Kelas</h6>
                        </div>
                        <div class="card-body">
                            <form class="user" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <input type="text" class="form-control" placeholder="Kelas Wali Kelas" maxlength="2"
                                        required name="kelas_walikelas" value="<?php echo $row[1]; ?>" readonly>
                                </div>
                                <div class="form-group">
                  ...
                  ?>
                                ...
                                <div class="form-group">
                                    <input type="submit" class="btn btn-primary btn-user btn-block"
                                        name="update_walikelas" value="Ubah Wali Kelas">
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <?php 
            if (isset($_POST['update_walikelas'])) {
              $nik_lama = $row[2];
              $kelas_walikelas = antiinjection($_POST['kelas_walikelas']);
              $nik_guru = antiinjection($_POST['nik_guru']);

              $update = mysqli_query($conn, "UPDATE wali_kelas SET nik = '$nik_guru' WHERE kode_wali_kelas = '$kode_walikelas'");

              if ($update) {
                $update_nik = mysqli_query($conn, "UPDATE guru SET kode_wali_kelas = NULL WHERE nik = '$nik_lama'");
                ...
                echo "<script>alert('Berhasil Mengubah Wali Kelas')</script>";
              } else {
                echo "<script>alert('Gagal Mengubah Wali Kelas')</script>";
              }
              ?>
